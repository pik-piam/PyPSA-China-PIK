...
def convert_row_units(row, target_unit_installation, target_unit_storage):
    ...
    # 先标准化 capacity_part 到小写(kw, mw, kwh, mwh)
    # 以便 capacity_factor_func 正确匹配
    def normalize_capacity_unit(cap):
        cap = cap.lower()
        # 把常见别名统一
        cap = cap.replace(" ", "")
        cap = cap.replace("²", "2")  # 万一有人写 m² ...
        return cap
    ...
    if norm_capacity in ["kw", "mw", "kwh", "mwh"] and norm_target in ["kw", "mw", "kwh", "mwh"]:
        ...
        new_capacity_part = norm_target  # 用目标容量字符串
    else:
        # 如果原容量不在 [kW, MW, kWh, MWh]，或目标容量为空
        # 则保持原字符串 capacity_part，不做容量换算
        cap_factor = 1.0
...
def plot_technologies_by_category(df, tech_colors=None, font_size=14):
    ...
    for i, tech in enumerate(technologies):
        ...
        # 4.6) 最后，手动同步坐标
        #   这样在保存时，右侧 y 轴大约是左侧的 7.8 倍
        #   如果后续还有绘图命令或 autoscale，会打破这个关系
