...
def records_to_data_frame(records:object)->gpd.GeoDataFrame:
    ...
    if not filtered.province.to_list()==sorted(PROV_NAMES):
        raise ValueError("Built cut-out does not have the right provinces - do your province lists have white spaces?")

    return filtered


def save_province_data(provinces_gdf:gpd.GeoDataFrame, crs:int= CRS,  output_file :os.PathLike= DEFAULT_SHAPE_OUTPATH):
    """save to file
