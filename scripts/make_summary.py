...
def make_summaries(networks_dict, paths, config):
    ...
    for label, filename in networks_dict.items():
        ...
        costs = load_costs(paths[1], config['costs'], config['electricity'], snakemake.wildcards.planning_horizons, Nyears)
...
if __name__ == "__main__":
    if 'snakemake' not in globals():
        ...
        snakemake = mock_snakemake('plot_summary', planning_horizons = 2060, opts = 'll')
    ...
    if config["foresight"] == "myopic":
        networks_dict = {(opts,planning_horizons):
            os.path.join(network_dir, f'postnetwork-{opts}-{topology}-{pathway}-{planning_horizons}.nc')
                         for opts in expand_from_wildcard("opts", config)
                         for planning_horizons in expand_from_wildcard("planning_horizons", config)
