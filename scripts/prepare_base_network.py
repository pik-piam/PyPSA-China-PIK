...
def transport_degree_factor(temperature,deadband_lower=15,deadband_upper=20,
    ...
    """Work out how much energy demand in vehicles increases due to heating and cooliƒng.
...
def prepare_data(network,date_range,planning_horizons):
    ...
    space_heat_demand_total = pd.read_csv("data/heating/SPH_2020.csv", index_col=0)
    space_heat_demand_total = space_heat_demand_total * 1e6
    space_heat_demand_total = space_heat_demand_total.squeeze()
    space_heating_per_hdd = space_heat_demand_total / (heat_demand_hdh.sum() * snakemake.config['frequency'])
    ...
    heat_demand = heat_demand*snakemake.config['heating_demand'][planning_horizons]
...
def prepare_network(config):
    ...
    if not isinstance(config['scenario']['co2_reduction'], tuple):
        ...
        if config['scenario']['co2_reduction'] is not None:
            ...
            co2_limit = 5.92*1e9 * (1 - config['scenario']['co2_reduction'][planning_horizons]) # Chinese 2020 CO2 emissions of electric and heating sector
    ...
    if config['add_H2_storage']:
        ...
        network.madd("Store",
                     ...
                     e_nom_extendable=True,
                     e_cyclic=True,
                     capital_cost=costs.at["hydrogen storage tank type 1","capital_cost"],
    ...
    if config["heat_coupling"]:
        ...
        central_fraction = pd.read_hdf("data/heating/DH_city:town_2020.h5")
